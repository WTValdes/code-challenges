...
function robberEncode(sentence) {
    let lower = 'bcdfghjklmnpqrstvwxyz'.split('');
    let upper = 'BCDFGHJKLMNPQRSTVWXYZ'.split('');
    return sentence.split('').map(a => lower.includes(a) ? a = (a + 'o' + a) : 
                                  upper.includes(a) ? a = (a + 'O' + a) : a).join('');
  }
